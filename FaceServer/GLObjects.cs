...
namespace GLObjects
{
    ...
    public class TextureFloat : IDisposable
    {
        ...
        public void LoadDepthFrame(Dopple.VideoFrame frame)
        {
            ...
            GL.TexImage2D(TextureTarget2d.Texture2D, 0, TextureComponentCount.R8,
                frame.DepthWidth, frame.DepthHeight, 0, PixelFormat.Red, PixelType.Float,
                frame.depthData);
    ...
    class TextureYUV : IDisposable
    {
        ...
        public void LoadImageFrame(Dopple.VideoFrame frame)
        {
            GL.ActiveTexture(TextureUnit.Texture0);
            GL.BindTexture(TextureTarget.Texture2D, TextureNameY);
            int ySize = frame.ImageHeight * frame.ImageWidth;
            byte[] yData = new byte[ySize];
            System.Buffer.BlockCopy(frame.imageData, 0, yData, 0, ySize);

            GL.TexImage2D(TextureTarget2d.Texture2D, 0, TextureComponentCount.R8,
                frame.ImageWidth, frame.ImageHeight, 0, PixelFormat.Red, PixelType.UnsignedByte,
                yData);
            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapS, (int)TextureWrapMode.ClampToBorder);
            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapT, (int)TextureWrapMode.ClampToBorder);
            ...
            int uvWidth = (frame.ImageWidth / 2);
            int uvHeight = (frame.ImageHeight / 2);
            int uvSize = uvHeight * uvWidth * 2;
            byte[] uvData = new byte[uvSize];
            System.Buffer.BlockCopy(frame.imageData, ySize, uvData, 0, uvSize);
